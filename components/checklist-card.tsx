...
export function ChecklistCard({ items }: ChecklistCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
      <h2 className="text-xl font-semibold text-slate-800 mb-4">세부 체크리스트</h2>
      <ul className="space-y-3" role="list">
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              <span
                className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold flex-shrink-0 ${config.bg} ${config.color}`}
                aria-hidden="true"
              >
                {config.icon}
              </span>
              <div className="flex-1 min-w-0">
                ...
                <span className="text-base text-slate-600">{item.value}</span>
              </div>
            </li>
          );
        })}
